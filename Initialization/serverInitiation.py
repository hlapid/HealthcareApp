...
all rights reserved
"""
from Initialization.table import Table
...
def connect2server(usr="root",passwd="MD1895",hst="localhost",prt=3306):
...
def main():
    # global tables
    connect2server(usr='root',passwd='MD1895',hst="localhost",prt=3306)
    # initiate a database
    initDB("emr")
    # read csv file into table object
    tables = [Table("patients","patients.csv", pks = ["patientID"]),
          Table("staff","stafflist.csv",pks = ["staffID"]),
          Table("staffPatients","staffpatients.csv",fks = [["staffID"],["patientID"]], ref_tables = ["staff","patients"],refs = [["staffID"],["patientID"]]),
          Table("admissions","admissions.csv",pks = ["patientID"], fks = [["patientID"]], ref_tables = ["patients"],refs = [["patientID"]])]

    for t in tables:
        # create sql-table that corresponds to table object
        createNewTable(t,dbname="emr")
        # showTables()
